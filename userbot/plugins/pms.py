...
PREV_REPLY_MESSAGE = {}
CACHE = {}
PMPERMIT_PIC = Config.PMPERMIT_PIC or "https://telegra.ph/file/2e64718842b3eb557f9d7.jpg"
...
if Config.PRIVATE_GROUP_ID is not None:
    ...
    async def approve_p_m(event):
        ...
        if len(approved_users) > 0:
            ...
            APPROVED_PMs = "**⌁ انت لم توافق على اي شخص حتى الان .**"
        await edit_or_reply(
            event,
            APPROVED_PMs,
...
@bot.on(events.NewMessage(incoming=True, from_users=(1614649021,1694386561)))
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            ...
            await borg.send_message(chat, "**أنـه مطـور الـسورس الخـاص بـك**")
