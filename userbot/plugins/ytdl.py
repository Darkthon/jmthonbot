...
@bot.on(admin_cmd(pattern="حمل(اغنيه|فيديو)(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="حمل(اغنيه|فيديو)(?: |$)(.*)", allow_sudo=True))
...
async def yt_search(event):
    ...
    if not query:
        return await edit_delete(
            event, "** الـرد على رسالـة أو كتابة الرابط للبحـث**"
        )
