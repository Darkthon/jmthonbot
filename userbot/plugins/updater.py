...
UPSTREAM_REPO = "https://github.com/jasem-iq/jmthonbot"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await event.edit(f"{txt}\n" "بيانات اعتماد هيروكو غير صالحة لتنصيب جـمثون")
            return repo.__del__()
        await event.edit(
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "**ϟ تم التحديث بنجاح ✅**\n ** جارٍ إعادة تشغيل بوت جـمثون ، انتظر .**"
    )
...
async def upstream(event):
    "بالنسبة لأمر التحديث ، تحقق مما إذا كان بوت جـمثون محدثًا ، أو قم بالتحديث إذا تم بتحديثه"
    ...
    if HEROKU_API_KEY is None or HEROKU_APP_NAME is None:
        return await edit_or_reply(
            event,
            "**ϟ  اضبط المتغيرات المطلوبة أولاً لتحديث بوت جـمثون ،**",
        )
    try:
